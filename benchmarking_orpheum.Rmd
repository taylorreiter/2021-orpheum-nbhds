...
```{r read_and_format_multiqc, eval = F}
# Control
nuc_reads_vs_aa_ref <- read_tsv("outputs/rgnv_sgc_original_paladin/multiqc_data/multiqc_samtools_flagstat.txt") %>%
  select(Sample, 
         nbhd_reads = flagstat_total, 
         nbhd_reads_mapped_aa_ref = mapped_passed,
         nbhd_reads_mapped_aa_ref_pct = mapped_passed_pct)

# PLASS DB
orph_aa_plass_k7_vs_aa_ref <- read_tsv("outputs/aa_paladin/plass_assembly/protein_ksize7/multiqc_data/multiqc_samtools_flagstat.txt") %>%
  select(Sample, 
         orph_aa_plass_k7_reads = flagstat_total, 
         orph_aa_plass_k7_reads_mapped_aa_ref = mapped_passed,
         orph_aa_plass_k7_reads_mapped_aa_ref_pct = mapped_passed_pct) %>%
  left_join(nuc_reads_vs_aa_ref) %>%
  mutate(orph_aa_plass_k7_pct_of_nbhd = (orph_aa_plass_k7_reads/nbhd_reads)*100,
         orph_aa_plass_k7_pct_of_nbhd_mapped = (orph_aa_plass_k7_reads_mapped_aa_ref/nbhd_reads)*100)

orph_aa_plass_k10_vs_aa_ref <- read_tsv("outputs/aa_paladin/plass_assembly/protein_ksize10/multiqc_data/multiqc_samtools_flagstat.txt") %>%
  select(Sample, 
         orph_aa_plass_k10_reads = flagstat_total, 
         orph_aa_plass_k10_reads_mapped_aa_ref = mapped_passed,
         orph_aa_plass_k10_reads_mapped_aa_ref_pct = mapped_passed_pct) %>%
  left_join(nuc_reads_vs_aa_ref) %>%
  mutate(orph_aa_plass_k10_pct_of_nbhd = (orph_aa_plass_k10_reads/nbhd_reads)*100,
         orph_aa_plass_k10_pct_of_nbhd_mapped = (orph_aa_plass_k10_reads_mapped_aa_ref/nbhd_reads)*100)

orph_nnc_plass_k7_vs_aa_ref <- read_tsv("outputs/nuc_noncoding_bwa/plass_assembly/protein_ksize7/multiqc_data/multiqc_samtools_flagstat.txt") %>%
   replace_na(list(mapped_passed_pct = 0)) %>%
  select(Sample, 
         orph_nnc_plass_k7_reads = flagstat_total, 
         orph_nnc_plass_k7_reads_mapped_aa_ref = mapped_passed,
         orph_nnc_plass_k7_reads_mapped_aa_ref_pct = mapped_passed_pct) %>%
  left_join(nuc_reads_vs_aa_ref) %>%
  mutate(orph_nnc_plass_k7_pct_of_nbhd = (orph_nnc_plass_k7_reads/nbhd_reads)*100,
         orph_nnc_plass_k7_pct_of_nbhd_mapped = (orph_nnc_plass_k7_reads_mapped_aa_ref/nbhd_reads)*100)

orph_nnc_plass_k10_vs_aa_ref <- read_tsv("outputs/nuc_noncoding_bwa/plass_assembly/protein_ksize10/multiqc_data/multiqc_samtools_flagstat.txt")%>%
   replace_na(list(mapped_passed_pct = 0)) %>%
  select(Sample, 
         orph_nnc_plass_k10_reads = flagstat_total, 
         orph_nnc_plass_k10_reads_mapped_aa_ref = mapped_passed,
         orph_nnc_plass_k10_reads_mapped_aa_ref_pct = mapped_passed_pct) %>%
  left_join(nuc_reads_vs_aa_ref) %>%
  mutate(orph_nnc_plass_k10_pct_of_nbhd = (orph_nnc_plass_k10_reads/nbhd_reads)*100,
         orph_nnc_plass_k10_pct_of_nbhd_mapped = (orph_nnc_plass_k10_reads_mapped_aa_ref/nbhd_reads)*100)

# ROARY DB

orph_aa_roary_k7_vs_aa_ref <- read_tsv("outputs/aa_paladin/roary_with_megahit_and_isolates/protein_ksize7/multiqc_data/multiqc_samtools_flagstat.txt") %>%
  select(Sample, 
         orph_aa_roary_k7_reads = flagstat_total, 
         orph_aa_roary_k7_reads_mapped_aa_ref = mapped_passed,
         orph_aa_roary_k7_reads_mapped_aa_ref_pct = mapped_passed_pct) %>%
  left_join(nuc_reads_vs_aa_ref) %>%
  mutate(orph_aa_roary_k7_pct_of_nbhd = (orph_aa_roary_k7_reads/nbhd_reads)*100,
         orph_aa_roary_k7_pct_of_nbhd_mapped = (orph_aa_roary_k7_reads_mapped_aa_ref/nbhd_reads)*100)

orph_aa_roary_k10_vs_aa_ref <- read_tsv("outputs/aa_paladin/roary_with_megahit_and_isolates/protein_ksize10/multiqc_data/multiqc_samtools_flagstat.txt") %>%
  select(Sample, 
         orph_aa_roary_k10_reads = flagstat_total, 
         orph_aa_roary_k10_reads_mapped_aa_ref = mapped_passed,
         orph_aa_roary_k10_reads_mapped_aa_ref_pct = mapped_passed_pct) %>%
  left_join(nuc_reads_vs_aa_ref) %>%
  mutate(orph_aa_roary_k10_pct_of_nbhd = (orph_aa_roary_k10_reads/nbhd_reads)*100,
         orph_aa_roary_k10_pct_of_nbhd_mapped = (orph_aa_roary_k10_reads_mapped_aa_ref/nbhd_reads)*100)

orph_nnc_roary_k7_vs_aa_ref <- read_tsv("outputs/nuc_noncoding_bwa/roary_with_megahit_and_isolates/protein_ksize7/multiqc_data/multiqc_samtools_flagstat.txt") %>%
   replace_na(list(mapped_passed_pct = 0)) %>%
  select(Sample, 
         orph_nnc_roary_k7_reads = flagstat_total, 
         orph_nnc_roary_k7_reads_mapped_aa_ref = mapped_passed,
         orph_nnc_roary_k7_reads_mapped_aa_ref_pct = mapped_passed_pct) %>%
  left_join(nuc_reads_vs_aa_ref) %>%
  mutate(orph_nnc_roary_k7_pct_of_nbhd = (orph_nnc_roary_k7_reads/nbhd_reads)*100,
         orph_nnc_roary_k7_pct_of_nbhd_mapped = (orph_nnc_roary_k7_reads_mapped_aa_ref/nbhd_reads)*100)

orph_nnc_roary_k10_vs_aa_ref <- read_tsv("outputs/nuc_noncoding_bwa/roary_with_megahit_and_isolates/protein_ksize10/multiqc_data/multiqc_samtools_flagstat.txt")%>%
   replace_na(list(mapped_passed_pct = 0)) %>%
  select(Sample, 
         orph_nnc_roary_k10_reads = flagstat_total, 
         orph_nnc_roary_k10_reads_mapped_aa_ref = mapped_passed,
         orph_nnc_roary_k10_reads_mapped_aa_ref_pct = mapped_passed_pct) %>%
  left_join(nuc_reads_vs_aa_ref) %>%
  mutate(orph_nnc_roary_k10_pct_of_nbhd = (orph_nnc_roary_k10_reads/nbhd_reads)*100,
         orph_nnc_roary_k10_pct_of_nbhd_mapped = (orph_nnc_roary_k10_reads_mapped_aa_ref/nbhd_reads)*100)

```

```{r combine_multiqc, eval = F}
all_multiqc <- Reduce(function(...) merge(..., by = c("Sample", "nbhd_reads", "nbhd_reads_mapped_aa_ref", "nbhd_reads_mapped_aa_ref_pct"), all.x=TRUE), 
       list(orph_aa_plass_k7_vs_aa_ref,
            orph_aa_plass_k10_vs_aa_ref,
            orph_nnc_plass_k7_vs_aa_ref,
            orph_nnc_plass_k10_vs_aa_ref, 
            orph_aa_roary_k7_vs_aa_ref,
            orph_aa_roary_k10_vs_aa_ref, 
            orph_nnc_roary_k7_vs_aa_ref,
            orph_nnc_roary_k10_vs_aa_ref))
```
