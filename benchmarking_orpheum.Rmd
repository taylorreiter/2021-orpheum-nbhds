...
sys_glob_path = "outputs/orpheum/roary_with_megahit_and_isolates/protein_ksize7/*json"
...
plot_predicted_nnc_and_flagstat_nnc <- function(orpheum_json, flagstat_path, ksize, alphabet, database, noncoding_estimate){
  ...
  plt <- ggplot(tmp, aes(x = reorder(Sample, non_coding_pct), y = percent,
    ...
    labs(title = paste0("db = ", database, ", alphabet = ", alphabet, ", k = ", ksize), 
         ...
         x = "genome query neighborhood", fill = "mapped to\nnucleotide CDS") +
...
all_old_plots <- function(flagstat_path_aa, flagstat_path_nnc, flagstat_path_nc,
                          ...
                          ksize, nbhd_reads, database, alaphabet){
...
all_old_plots(flagstat_path_aa = "outputs/aa_paladin/roary_with_megahit_and_isolates/protein_ksize7/multiqc_data/multiqc_samtools_flagstat.txt", 
              ...
              database = "roary", alaphabet = "protein")
```

```{r try_new_figure_panels}
tmp1 <- plot_orpheum_translation_frames(orpheum_trans = orph_trans_roary_k7, ksize = 7, 
                                alphabet = "protein", database = "roary")

tmp2 <- plot_predicted_nnc_vs_flagstat_nnc(orpheum_json = orph_roary_k7, 
                                   flagstat_path = "outputs/nuc_noncoding_bwa/roary_with_megahit_and_isolates/protein_ksize7/multiqc_data/multiqc_samtools_flagstat.txt", 
                                   ksize = 7, alphabet = "protein",
                                   database = "roary_with_megahit_and_isolates",
                                   noncoding_estimate = 10.2)

ggarrange(tmp1, tmp2, ncol = 2)
```
